...
exports.read = function(req, res) {
	console.log('Opportunity from request: ' + req.opportunity);
...
exports.list = function(req, res) { 
	var onlyPublished = req.query.publishedOnly;
	console.log('onlyPublished from req.params: ' + onlyPublished);
	var filter = {};
	if(onlyPublished) {
		filter = {status : 'Published'};
		console.log('filter: ' + filter);
    }
	Opportunity.find(filter).sort('-created').populate('user', 'displayName').exec(function(err, opportunities) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.opportunityByID = function(req, res, next, id) { 
	console.log('Inside opportunityByID ' + id);
